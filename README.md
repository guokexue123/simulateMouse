...
3. **找未读会话**：先锚定左侧的**「聊天」分组**——只扫这个标题下方、下一个分组标题
   上方的会话，收藏夹、频道、团队都不看。再去掉嵌套重复项（外层容器和内层容器
   都符合条件时只留最外层，否则会话数会莫名翻倍）。然后用四种方式判断未读，
   **任意一种命中就算未读**：

   | 方式 | 依据 | 适用情况 |
   |---|---|---|
   | `text` | 控件名里有"未读""unread"等字样 | **实测有效**，新版 Teams 会把控件名写成"未读消息 群组聊天 XXX" |
   ...
   **默认只启用 `text`**——实测这个 Teams 版本把未读写进控件名了，一条就够，
   ...
   （外接矩形 3~26 像素、接近正方形、填充率 ≥45%），但仍然受主题配色和窗口遮挡
   影响，需要时再手动 `--detect text,dot` 打开。

   会话名会自动清理：`未读消息 群组聊天 口岸GTW测试交流群 已静音` → `口岸GTW测试交流群`，
   同时过滤掉"查看更多"这类非会话项。
4. **读消息正文**：依次点开每个未读会话，取窗口右侧（宽度 45% 以线）同一父控件下
   的那组条目，按纵坐标排序还原对话顺序。Teams 的无障碍描述里通常已经带了
   "谁 说了 什么 时间"，直接取控件名即可。
...
- **双击位置**：默认在鼠标当前所在位置双击。如果光标停在某个按钮或链接上，
  ...
  指定一个安全坐标。
  脚本会自动避开 Teams 窗口：光标落在窗口范围内时，双击会挪到窗口外侧再点，
  并打印一行提示。**这一步是必要的**——点在会话列表上会切换选中项、弹出悬停
  菜单，而选中行的字重和悬停行多出来的"⋯"按钮都会被误判成未读，等于保活动作
  在干扰自己要测量的东西。窗口占满整个屏幕时挪不开，那就只能用 `--no-click`
  或 `--click-pos` 指定一个安全位置。
- **当前选中的会话不参与 `bold` / `structure` 判断**：Teams 给选中行加了高亮样式，
  字重和结构都跟别人不一样。它正开着，本来也不会有未读。
